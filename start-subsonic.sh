...
Initialise(){
   ...
   if [ -z "${SUBSONIC_CONTEXT_PATH}" ]; then
      echo "$(date '+%Y-%m-%d %H:%M:%S') INFO:    SUBSONIC_CONTEXT_PATH not set, defaulting to /subsonic"
      SUBSONIC_CONTEXT_PATH="/subsonic"
   else
   ...
   if [ ! -f "${CONFIGDIR}/https/subsonic.key" ]; then
   ...
   if [ ! -f "${CONFIGDIR}/https/subsonic.csr" ]; then
   ...
   if [ ! -f "${CONFIGDIR}/https/subsonic.crt" ]; then
